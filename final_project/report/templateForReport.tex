...
\begin{figure}
	\centering
	\includegraphics[width=\linewidth]{SDDR_linear vs. squared_sigma_07+.pdf}
	\includegraphics[width=\linewidth]{SDDR_squared vs. cubic_sigma_07+.pdf}
	\caption{Computation of the SDDR}
	\label{fig:sddr}
\end{figure}
\subsubsection{\textbf{Bayes-Complexity}}
As another measure of model selection, the \textsc{Bayesian} complexity $\mathcal{C}_b$ was introduced. Having sampled in parameter space $\btheta^{(t)}$, equation \eqref{eq:Bayes_Complexity_alt} is evaluated instantly as \cite{wuerz} $$\mathcal{C}_b=\frac{1}{N_S}\sum_{t=0}^{N_S-1}\left(\sum_{k=0}^{N_y-1}\frac{[y_k-f()]}{}\right)$$
\section{Discussion}
